// ─────────────────────────────────────────────────────────────────────────────
// src/data/manualContent.tsx — Fuente única del contenido del manual
// Consumida por DOS lugares (sin duplicar texto):
//   · src/pages/Manual.tsx        → renderiza el manual completo en /manual
//   · src/components/help/HelpPanel.tsx → muestra por pantalla el bloque relevante
// El contenido vive aquí como bloques JSX nombrados; editarlo aquí actualiza ambos.
// Espejo de prosa plano: docs/manual_usuario.md
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Modelo de secciones (índice + scroll-spy en /manual) ──────────────────────
...
export function SubHead({ icon: Icon, children }: { icon: LucideIcon; children: ReactNode }) {
  return (
...
export function Flow({ items }: { items: { label: string; icon: LucideIcon }[] }) {
  return (
...
// Paso de la secuencia de ejemplo (timeline vertical) — solo usado en el bloque 'ejemplo'
...
// ── Bloques de contenido (fuente única de prosa) ──────────────────────────────
// Cada bloque es un fragmento autocontenido; se componen por sección en /manual
// y se seleccionan por pantalla en el HelpPanel.

export const BLOCKS: Record<string, ReactNode> = {
  // ── Intro (badge PAPER) — se muestra en /manual antes de la sección 01
  //    y en la ayuda contextual del Dashboard.
  'paper-mode': (
...
// Orden de bloques por sección (para /manual). 'paper-mode' se renderiza como
// intro antes de la sección 01, por eso no aparece aquí.
export const SECTION_BLOCKS: Record<string, string[]> = {
  'primer-dia-en-tradeos':       ['dashboard-overview', 'colores', 'riesgo'],
  'flujo-para-generar-opciones': ['flow-02', 'screener-run', 'screener-results', 'research-agent', 'enviar-plan'],
  'flujo-para-operar':           ['flow-03', 'plan-vuelo', 'trading-order', 'confirmar', 'journal-registro', 'post-mortem'],
  'referencia-rapida':           ['pantallas', 'reglas', 'glosario'],
  'ejemplo-completo':            ['ejemplo'],
}

// ── Ayuda contextual por pantalla ─────────────────────────────────────────────
// Cada tema selecciona un subconjunto de BLOCKS (solo lo relevante a esa pantalla).
...
export const HELP_TOPICS: Record<string, HelpTopic> = {
  '/': {
    title:  'Primer día en TradeOS',
    icon:   LayoutDashboard,
    blocks: ['paper-mode', 'dashboard-overview', 'colores', 'riesgo'],
  },
  '/trading': {
    title:  'Operar en Trading',
    icon:   TrendingUp,
    blocks: ['trading-order', 'confirmar'],
  },
  '/screener': {
    title:  'Generar opciones: Screener',
    icon:   Search,
    blocks: ['screener-run', 'screener-results'],
  },
  '/research': {
    title:  'Research Agent',
    icon:   Telescope,
    blocks: ['research-agent'],
  },
  '/journal': {
    title:  'Journal y post-mortem',
    icon:   BookOpen,
    blocks: ['journal-registro', 'post-mortem'],
  },
  '/flight-plan': {
    title:  'Plan de Vuelo',
    icon:   ClipboardList,
...
// ── Estilos del contenido (compartidos por /manual y HelpPanel) ───────────────
// Cubre subheads, tarjetas de paso, listas, callouts, flujos, ejemplo, código y
// tablas. El wrapper que los contiene debe llevar la clase `manual-md` (para los
// selectores scoped de código y tablas). Los estilos de "chrome" de la página
// (índice lateral, hero, headings h2, footer) viven en Manual.tsx.

export function ManualContentStyles() {
  return (
    <style>{`
      /* ── Sub-encabezado ─────────────────────────────────────────────── */
      .m-subhead {
