...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET" && subPath === "/account") {
      const res  = await fetch(`${baseUrl}/v2/account`, { headers: alpacaHeaders })
    ...
    if (req.method === "GET" && subPath === "/positions") {
      const res  = await fetch(`${baseUrl}/v2/positions`, { headers: alpacaHeaders })
      const data = await res.json()
      ...
      const totalEquity = (data as AlpacaPosition[]).reduce(
        (sum: number, p: AlpacaPosition) => sum + parseFloat(p.market_value ?? "0"),
        0
      )
    ...
    if (req.method === "POST" && subPath === "/orders") {
      ...
      const {
        ...
        risk_amount, portfolio_weight_at_order,
        risk_reward_ratio,
      } = body

      // Validaciones básicas
      if (!symbol) return errJson("symbol requerido")
      if (!side || !["buy", "sell"].includes(side)) return errJson("side debe ser 'buy' o 'sell'")
      ...
      if ((order_type === "limit" || order_type === "stop_limit") && limit_price) {
      ...
      const { data: savedOrder, error: dbError } = await supabase
        ...
        .insert({
          ...
          asset_class:              alpacaOrder.asset_class === "crypto" ? "crypto" : "equity",
